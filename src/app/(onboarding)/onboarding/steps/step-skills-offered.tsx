...
import { saveSkillsOfferedAction } from "../actions";
import { cn } from "@/lib/utils";

const SUGGESTIONS = [
  "photography", "design", "videography", "music", "writing",
  "hair", "makeup", "styling", "coding", "fitness",
  "illustration", "editing", "copywriting", "animation", "branding",
  "yoga", "social media", "tattooing", "cooking", "nail art",
];

export function StepSkillsOffered({ defaultValue }: { defaultValue: string[] }) {
  ...
  return (
    <form action={saveSkillsOfferedAction} className="flex flex-1 flex-col">
      {/* Hidden form inputs — one per selected tag */}
      {tags.map((t) => (
        <input key={t} type="hidden" name="skills" value={t} />
      ))}
      ...
      <StepShell
        ...
        subtitle="Tap anything that applies to you. Add up to 12."
        skipHref="/onboarding?step=3"
        footer={<ContinuePill>That&apos;s what I&apos;ve got</ContinuePill>}
      >
